...
import decodeB64ToUint8Array from './decodeB64ToUint8Array';

import LineStream from './line-stream';
import ParseStream, { AddParser, AddTagMapper } from './parse-stream';
...
const camelCaseKeys = function (attributes: Record<string, AnyType>) {
...
const setHoldBack = function (this: Parser, manifest: Record<string, AnyType>) {
...
export default class Parser extends Stream {
  ...
  manifest: Record<string, AnyType>;
  constructor() {
    super();
    ...
    const uris: Array<Record<string, AnyType>> = [];
    let currentUri: Record<string, AnyType> = {};
    // if specified, the active EXT-X-MAP definition
    let currentMap: Record<string, AnyType>;
    ...
    const dateRangeTags: Record<string, AnyType> = {};
    ...
    this.parseStream.on('data', function (entry: Record<string, AnyType>) {
      let mediaGroup;
      let rendition: Record<string, AnyType>;
            ...
            (
              (
                ({
                  ...
                  byterange() {
                    const byterange: Record<string, AnyType> = {};
                  ...
                  key() {
                    ...
                    if (
                      entry.attributes.KEYFORMAT === 'com.microsoft.playready'
                    ) {
                      this.manifest.contentProtection =
                        this.manifest.contentProtection || {};
                    ...
                    if (entry.attributes.KEYFORMAT === widevineUuid) {
                      ...
                      if (
                        VALID_METHODS.indexOf(entry.attributes.METHOD) === -1
                      ) {
                        this.trigger('warn', {
                  ...
                  'media-sequence'() {
                    if (!isFinite(entry.number)) {
                      this.trigger('warn', {
                        message:
                          'ignoring invalid media sequence: ' + entry.number,
                      });
                      return;
                    }
                  ...
                  part() {
                    ...
                    const part: Record<string, AnyType> = camelCaseKeys(
                      entry.attributes
                    );
                    ...
                    if (this.manifest.renditionReports) {
                      this.manifest.renditionReports.forEach(
                        (r: Record<string, AnyType>, i: number) => {
                  ...
                  'server-control'() {
                    const attrs: Record<string, AnyType> =
                      (this.manifest.serverControl = camelCaseKeys(
                        entry.attributes
                      ));
                  ...
                  'preload-hint'() {
                    // parts are always specifed before a segment
                    const segmentIndex = this.manifest.segments.length;
                    const hint: Record<string, AnyType> = camelCaseKeys(
                      entry.attributes
                    );
                    ...
                    for (
                      let i = 0;
                      i < currentUri.preloadHints.length - 1;
                      i++
                    ) {
                  ...
                  daterange() {
                    ...
                    if (dateRange.duration && dateRange.endDate) {
                      ...
                      const newDateInSeconds = startDate.setSeconds(
                        startDate.getSeconds() + dateRange.duration
                      );
                    ...
                    if (!dateRangeTags[dateRange.id]) {
                      ...
                      for (const attribute in dateRangeTags[dateRange.id]) {
                        if (
                          dateRangeTags[dateRange.id][attribute] !==
                          dateRange[attribute]
                        ) {
                          this.trigger('warn', {
                            message:
  ...
  warnOnMissingAttributes_(
    identifier: string,
    attributes: Record<string, AnyType>,
    required: string[]
  ) {
   ...
   */
  addTagMapper(options: AddTagMapper) {
